...
商用Lora模块：DX-LR22
```
使用商用 LoRa 模块作为射频激励源，并由USRP B200接收Backscatter信号。采用滑动窗口dechirp、FFT峰值检测以及上升沿周期估计完成TMR开关周期计算。
...
LoRa信号作为Backscatter标签的外部射频激励。标签通过振荡器产生频移信号，并使用TMR开关控制频移信号，实现OOK调制，再输入至RF开关控制负载阻抗，从而对反射信号进行频移与调制。在接收端通过估计OOK频率恢复风速信息。
...
3. 启动商用LoRa发射模块；
4. Backscatter标签供电；
5. 将 `center_freq` 设置为实际背散射频移后的LoRa信号频率；
6. 启动Flowgraph采集IQ数据。
...
程序检测OOK上升沿后，根据相邻上升沿间隔估算标签的开关频率。
